...
void OnPointCloud(const sensor_msgs::PointCloud2ConstPtr &ros_pc2) {
    ...
    // header.stamp = ros::Time::now();  //  应该是bag时间，这里不是实时运行的
    ...
    *cloud_nonground = *cloud;
    ...
    autosense::common::publishClustersCloud<autosense::PointI>(
        clusters_pub_, header, cloud_clusters);
...
int main(int argc, char **argv) {
    ...
    std::string sub_pc_topic, pub_pcs_segmented_topic, pub_pc_clusters_topic;
    ...
    private_nh.getParam(param_ns_prefix_ + "/pub_pc_clusters_topic",
                        pub_pc_clusters_topic);  // add by yujie.jin
