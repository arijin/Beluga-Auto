...
#include "roi_filters/roi.hpp"  // roi::MapROIFilter
...
bool use_map_roi_filter_;
std::vector<autosense::MapROIParams> params_map_roi_;
...
ros::Publisher segments_filtered_pub_;
...
void OnSegmentClouds(
    ...
    ROS_INFO("Clusters size: %d at %lf.", segments_msg->clouds.size(),
             kTimeStamp);
    std_msgs::Header header = segments_msg->header;
    // header.frame_id = local_frame_id_;
    // header.stamp = segments_msg->header.stamp;

    // initial coarse segments directly from segment node or after
    // classified by learning node
    ...
    /**
     * @brief Use roi range(e.g. HDmap) information to filter segmentation
     * @note
     *  <1> project coarse segmentation into World coordinate
     *  <2> get filtered objectd by roi range (in world coordinate)
     *  <3> project back to current Velodyne coordinate
     */
    if (use_map_roi_filter_) {
        autosense::common::publishRangeMapROI(range_roi_pub_, header,
                                              params_map_roi_);

        std::vector<autosense::ObjectPtr> filtered_objects;
        autosense::roi::MapROIFilter(objects, params_map_roi_, *velo2world,
                                     &filtered_objects);
        objects.clear();
        objects = filtered_objects;
    }
    autosense::common::publishObjectsMarkers(segments_filtered_pub_, header,
                                             autosense::common::MAGENTA.rgbA,
                                             objects);
...
int main(int argc, char **argv) {
    ...
    std::string pub_segments_coarse_topic, pub_segments_filtered_topic,
        pub_segments_predict_topic, pub_segments_topic;
    private_nh.getParam(param_ns_prefix_ + "/pub_segments_coarse_topic",
                        pub_segments_coarse_topic);
    private_nh.getParam(param_ns_prefix_ + "/pub_segments_filtered_topic",
                        pub_segments_filtered_topic);
    ...
    // range map roi
    private_nh.param<bool>(param_ns_prefix_ + "/use_map_roi_filter",
                           use_map_roi_filter_, false);
    params_map_roi_ =
        autosense::common::getMapROIParams(private_nh, param_ns_prefix_);
    std::string pub_range_roi_topic;
    private_nh.getParam(param_ns_prefix_ + "/pub_range_roi_topic",
                        pub_range_roi_topic);
    range_roi_pub_ = private_nh.advertise<geometry_msgs::PolygonStamped>(
        pub_range_roi_topic, 1);
    ...
    segments_filtered_pub_ =
        private_nh.advertise<visualization_msgs::MarkerArray>(
            pub_segments_filtered_topic, 1);
